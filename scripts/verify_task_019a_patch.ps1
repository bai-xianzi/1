# 脚本总览：`scripts/verify_task_019a_patch.ps1` 负责执行项目验证、交付或运维流程。
...
& $Python `
  ".\scripts\validate_task_019a_market_state_input_contract.py" `
  --project-root $ProjectRoot

if ($LASTEXITCODE -ne 0) {
    throw "TASK_019A合同验证失败。"
}

& $Python -m unittest tests.test_market_state_inputs -v

if ($LASTEXITCODE -ne 0) {
    throw "TASK_019A专项测试失败。"
}
