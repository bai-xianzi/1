# 脚本总览：`scripts/verify_task_019d_patch.ps1` 负责执行项目验证、交付或运维流程。
...
& $Python `
  ".\scripts\validate_task_019d_market_state_scoring.py" `
  --project-root $ProjectRoot
if ($LASTEXITCODE -ne 0) {
    throw "TASK_019D离线验证失败。"
}

& $Python -m unittest tests.test_market_state_scoring -v
if ($LASTEXITCODE -ne 0) {
    throw "TASK_019D专项测试失败。"
}
