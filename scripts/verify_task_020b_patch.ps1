# 脚本总览：`scripts/verify_task_020b_patch.ps1` 负责执行项目验证、交付或运维流程。
...
if ($LASTEXITCODE -ne 0) {
    throw "TASK_020B协议验证失败。"
}

& $Python -m unittest tests.test_provider_plugin_protocol -v
if ($LASTEXITCODE -ne 0) {
    throw "TASK_020B专项测试失败。"
}
