# 本文件核心功能：实现 `validate_task_022_provider_activation.py` 在TASK_022至TASK_024范围内的单一任务职责。
...
# 本段代码核心功能：定义 `require`，完成 `require` 对应的单一业务步骤并返回明确结果。
# - 输入：参数为 `condition、message`；路径和外部文本按UTF-8处理，安全开关必须由显式配置提供。
...
def require(condition: bool, message: str) -> None:
    # 本段代码核心功能：根据条件 `not condition` 选择安全分支。
...
# 本段代码核心功能：定义 `contains_string_literal`，完成 `contains_string_literal` 对应的单一业务步骤并返回明确结果。
# - 输入：参数为 `source、target`；路径和外部文本按UTF-8处理，安全开关必须由显式配置提供。
# - 处理：只执行函数名对应的单一职责；缺字段、非法状态或越界值立即失败，不做静默猜测。
# - 输出：返回类型为 `bool`；测试函数通过断言表达通过或失败，不产生生产副作用。
# - 常量依据：任务号、状态枚举、零网络/零交易计数和候选优先级来自TASK_022至TASK_024权威合同。
# - 为什么这样写：显式输入输出和失败模式便于教学排查，也让官方交易所或券商SDK通过薄适配器接入。
...
# 本段代码核心功能：定义 `main`，执行 `validate_task_022_provider_activation.py` 的命令行主流程并返回标准退出码。
