# 脚本总览：`scripts/verify_task_019c_patch.ps1` 负责执行项目验证、交付或运维流程。
...
& $Python `
  ".\scripts\validate_task_019c_real_feature_acceptance.py" `
  --project-root $ProjectRoot
if ($LASTEXITCODE -ne 0) {
    throw "TASK_019C离线合同验证失败。"
}

& $Python `
  -m unittest `
  tests.test_market_state_real_acceptance `
  -v
if ($LASTEXITCODE -ne 0) {
    throw "TASK_019C专项测试失败。"
}
