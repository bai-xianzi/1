...
## TASK_024E：本地B2C页面与应用后端

这个功能让用户在本机页面中选择数据Provider、填写官方连接字段、安全保存密钥、查看连接状态并发起只读测试，不再手工修改配置文件。
...
页面只监听`127.0.0.1`，变更请求必须携带CSRF令牌；秘密字段不回填、不进入浏览器持久化、普通JSON、日志、报告或Git。TASK_024E只建立统一页面与后端，真实Provider测试器由TASK_024F接入，交易能力继续`BLOCKED`。
